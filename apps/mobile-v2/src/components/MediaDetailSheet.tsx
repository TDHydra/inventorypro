// Station D2: ported from apps/mobile/src/components/MediaDetailSheet.tsx.
// Import mapping: ui kit/Alert/theme/SuggestInput → '@invenpro/ui'; useDbQuery/
// runInTransaction → '@invenpro/core'; db/queries/* → '../repos/*';
// isMediaUploadPending moved from sync/outbox into repos/media. v2 repos don't
// self-log, so each mutation here wraps runInTransaction(repo call +
// logMediaAction) with the exact old log rows (see ../media/mediaLog.ts).
import { useState, useMemo, useCallback, useEffect } from 'react';
import { View, Text, Image, TextInput, TouchableOpacity, StyleSheet } from 'react-native';
import {
  Alert, useTheme, useThemedStyles, type Theme,
  ModalSheet, AppInput, FieldLabel, PrimaryButton, SuggestInput,
} from '@invenpro/ui';
...
import { getAllJobs, Job } from '../repos/jobs';
import { logMediaAction } from '../media/mediaLog';
...
export function MediaDetailSheet({ mediaId, visible, onClose, onChanged }: Props) {
  ...
  const saveEdit = () => {
    if (!detail) return;
    const fields = { caption: caption.trim() || null, location_note: locationNote.trim() || null };
    runInTransaction(() => {
      updateMediaMeta(detail.id, fields);
      logMediaAction('media_updated', detail.id, user?.id ?? null, fields.location_note ?? fields.caption ?? null);
    });
  ...
  const confirmMove = (job: Job) => {
    ...
    Alert.alert('Move photo?', `Move this photo to ${job.name}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: () => {
          runInTransaction(() => {
            moveMediaToJob(detail.id, job.id);
            logMediaAction('media_updated', detail.id, user?.id ?? null, 'moved to another job');
          });
          setMoveOpen(false);
          onChanged();
        },
      ...
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: () => {
          runInTransaction(() => {
            ...
            logMediaAction('media_deleted', detail.id, user?.id ?? null, null);
          });
          onClose();
          onChanged();
        },
