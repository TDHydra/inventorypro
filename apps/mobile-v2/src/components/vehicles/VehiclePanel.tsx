...
import {
  Alert, Card, KeyValueRow, TypeBadge, StatusPill, SegmentedControl, PrimaryButton, FieldLabel,
  confirmSheet, VerticalLevelSlider, useThemedStyles, type Theme,
} from '@invenpro/ui';
...
import { getLocationById, retireVehicle, reactivateVehicle } from '../../repos/locations';
...
import { canManageVehicle, canLiftVehicleLockFor } from '../../repos/access';
 ...
 *    runInTransaction + its own appendLog('vehicle_state_changed', ...)
 *    using the returned note. checkInVehicle likewise no longer self-logs —
 *    onPrimaryPress's check-in branch builds 'vehicle_checkin' from the
 *    returned session. retireVehicle/reactivateVehicle (repos/locations.ts)
 *    are UNCHANGED here — they still self-log (kept deliberately, matching
 *    their siblings in that file — see repos/locations.ts's header note).
 */
interface Props {
  locationId: string;
...
export function VehiclePanel({ locationId, variant, onNavigate }: Props) {
  ...
  /** Wrap a state patch write + its 'vehicle_state_changed' log in one transaction. */
  function writeState(patch: Parameters<typeof upsertVehicleState>[1]) {
    runInTransaction(() => {
      const note = upsertVehicleState(locationId, patch, user?.id ?? null);
      appendLog({
  ...
  function setWaterTank(id: string) {
    ...
    writeState({ water_tank: id as WaterTank });
  }

  function setWasteTank(id: string) {
    if (isWriteBlocked()) return;
    writeState({ waste_tank: id as WasteTank });
  }
  ...
  async function onPrimaryPress() {
    ...
    if (action.kind === 'check_in') {
      runInTransaction(() => {
        const result = checkInVehicle(action.sessionId);
        if (!result) return;
        appendLog({
          action: 'vehicle_checkin',
          entity_type: 'location',
          entity_id: result.session.vehicle_location_id,
          user_id: user?.id ?? null,
          team_id: null,
      ...
      <Card variant="detail">
        ...
        {canManage && !locked && (
          (vehicle?.checkout_locked ? canBypassLock : true) ? (
            <Pressable
              onPress={() => {
                if (isWriteBlocked()) return;
                writeState({ checkout_locked: vehicle?.checkout_locked ? 0 : 1 });
              }}
              style={s.truckRow}
            >
              <StatusPill
          ...
          <>
            ...
            {locked ? (
              ...
              <VerticalLevelSlider
                ...
                onCommit={raw => {
                  if (isWriteBlocked()) return;
                  writeState({ debris_level: snapDebrisLevel(raw) });
                }}
              />
            )}
          </>
        ...
        {locked ? (
          ...
          <VerticalLevelSlider
            ...
            onCommit={raw => {
              if (isWriteBlocked()) return;
              writeState({ fuel_level: snapFuelLevel(raw) });
            }}
          />
        )}
      </Card>
