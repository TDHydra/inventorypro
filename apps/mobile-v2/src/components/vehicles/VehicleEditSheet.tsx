...
import {
  Alert, EntityEditSheet, TextField, StatusPill, FieldLabel, useThemedStyles, type Theme,
} from '@invenpro/ui';
...
import { getLocationById, upsertLocation } from '../../repos/locations';
import {
 ...
 *
 * Station C3 deviation from apps/mobile: upsertLocation already self-mirrors
 * (strips synced_at/type_id/booleans internally via its own .mirror() call —
 * repos/locations.ts), so handleSave() below just calls it directly instead
 * of hand-rolling the outbox strip the old app did. upsertVehicleState /
 * createServiceRecord no longer self-log (Station C3's no-self-log
 * conversion) — handleSave() builds 'vehicle_state_changed' /
 * 'vehicle_service_logged' appendLog calls itself from their returned notes.
 */
export function VehicleEditSheet({ locationId, visible, onClose }: Props) {
  ...
  function handleSave() {
    ...
    try {
      // Atomic: renamed location + the vehicle spec write land together.
    ...
    if (isEditor && odoValue != null && odoValue !== odoSeed) {
      try {
        runInTransaction(() => {
          const { note } = createServiceRecord({
