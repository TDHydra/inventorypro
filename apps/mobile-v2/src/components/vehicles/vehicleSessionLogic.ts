...
// ui/dateFieldLogic.ts. The DB layer (repos/vehicles.ts) and the panel both
// consume these so the close-only payload shape and the who-holds-it decision
// live in exactly one place.
//
// Station C3: verbatim port from apps/mobile/src/components/vehicles/
// vehicleSessionLogic.ts (zero imports, so import-mapping is a no-op).
