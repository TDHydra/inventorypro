// Station D2: ported from apps/mobile/src/components/MediaGallery.tsx.
// Import mapping: ui kit/Alert/theme → '@invenpro/ui'; useDbQuery/
// runInTransaction → '@invenpro/core'; db/queries/{media,rooms} →
// '../repos/{media,rooms}'. v2 repos don't self-log, so the delete here wraps
// runInTransaction(deleteMedia + logMediaAction) — old logMediaAction shape
// preserved in ../media/mediaLog.ts.
...
import { Alert, ModalSheet, AppInput, useTheme, useThemedStyles, type Theme } from '@invenpro/ui';
...
import { MediaRecord, getMediaForEntity, deleteMedia } from '../repos/media';
import { SearchablePicker, type PickerOption } from './SearchablePicker';
import { usePermission } from '../hooks/usePermission';
import { getRooms, addRoom } from '../repos/rooms';
    ...
    Alert.alert('Delete Photo', 'Delete this photo? This cannot be undone.', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: () => {
          runInTransaction(() => {
            deleteMedia(item.id);
            logMediaAction('media_deleted', item.id, user?.id ?? null, null);
          });
          setLightbox(null);
        },
      },
