// Pure client-side validation for the Change PIN sheet (Station D3, ported
// from apps/mobile/src/components/profile/pinChangeLogic.ts). Reuses the
// enrollment rules — validatePinFormat with the user's pin_length_required,
// isWeakPin — plus the two change-specific rules the server also enforces
// (different-from-current, matching confirmation) so the user gets instant
// per-field feedback before any network call. The server re-validates
// everything; this is UX, not security.

import { validatePinFormat, isWeakPin } from '@invenpro/core';
