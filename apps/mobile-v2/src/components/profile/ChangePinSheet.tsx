// Station D3: ported from apps/mobile/src/components/profile/ChangePinSheet.tsx.
// Import mapping per docs/REBUILD-PORTING.md: ui components + themedAlert →
// '@invenpro/ui'; auth/pin (changePinOnline) → '@invenpro/core'.
import { useEffect, useState } from 'react';
import { View, Text, StyleSheet } from 'react-native';
import type { Theme } from '@invenpro/ui';
import { EntityEditSheet, TextField, Alert, useThemedStyles } from '@invenpro/ui';
import { changePinOnline } from '@invenpro/core';
...
/**
 * My Profile → Change PIN. Current PIN + new PIN twice; client pre-validation
 * via validatePinChange (enrollment rules + different-from-current + confirm
 * match) with per-field errors, then POST /me/change-pin. Online-only, like
 * every PIN path — the PIN never touches the device DB. A 403 (wrong current
 * PIN) lands on the current-PIN field; server 400 reasons (weak /
 * same-as-current caught server-side) land on the new-PIN field.
 */
