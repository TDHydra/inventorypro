// Station D3: ported from apps/mobile/src/components/profile/ChangePhoneSheet.tsx.
// Same import mapping as ChangeEmailSheet (see its header).
...
/**
 * My Profile → Change phone. Format validation only (no SMS infra):
 * validatePhone normalizes (optional leading '+', 7–15 digits after stripping
 * separators), blank clears. PATCH /me/phone, then mirror into the local
 * users row (no outbox — the server committed it and bumped updated_at for
 * sync).
 */
export function ChangePhoneSheet({ visible, onClose, currentPhone }: Props) {
  ...
  const handleSave = async () => {
    ...
    if (user) updateUserLocal(user.id, { phone: v.value });
