// Station D3: ported from apps/mobile/src/components/profile/ChangeEmailSheet.tsx.
// Import mapping: ui components + themedAlert → '@invenpro/ui';
// db/queries/users (updateUserLocal) → '../../repos/users';
// sync/dataVersion (bumpDataVersion) → covered by updateUserLocal's own
// queueTableBump, so no explicit bump here (deviation from the old sheet).
import { useEffect, useState } from 'react';
import { View, Text, StyleSheet } from 'react-native';
import type { Theme } from '@invenpro/ui';
import { ModalSheet, FormActions, TextField, Alert, useThemedStyles } from '@invenpro/ui';
import { useSession } from '../../hooks/useSession';
import { validateEmail } from '../../lib/validation';
import { requestEmailChangeCode, confirmEmailChange } from '../../api/me';
import { updateUserLocal } from '../../repos/users';
...
/**
 * My Profile → Change email. Multi-step, so it composes ModalSheet +
 * FormActions directly instead of EntityEditSheet (which closes on every
 * successful save — here only the LAST step closes). On success the confirmed
 * address is mirrored into the local users row (no outbox — the server
 * already committed it and bumped updated_at for sync).
 */
export function ChangeEmailSheet({ visible, onClose }: Props) {
  ...
  const finalize = async (withCode: boolean) => {
    ...
    if (user) updateUserLocal(user.id, { email });
