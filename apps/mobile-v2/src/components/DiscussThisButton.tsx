...
import { useThemedStyles, ModalSheet, KIT_HIT_SLOP } from '@invenpro/ui';
import { useDataVersion, syncNow } from '@invenpro/core';
...
// Station D1: ported from apps/mobile/src/components/DiscussThisButton.tsx
...
// referencing the entity. Shared so jobs/repairs/equipment can't drift apart
// — replaces the headerRight chat cuts those screens carried through Wave C.
export function DiscussThisButton({ kind, label, refText }: {
  ...
  const pick = useCallback((opt: PickerOption) => {
    if (!userId) return;
    // Same write-block guard as every other write affordance — during
