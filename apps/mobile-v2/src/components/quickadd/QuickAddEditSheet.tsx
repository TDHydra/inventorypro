...
import {
  Alert, confirmDestructive, ModalSheet, PrimaryButton, AppInput, FieldLabel,
  useTheme, useThemedStyles,
} from '@invenpro/ui';
import type { Theme } from '@invenpro/ui';
import { useSession } from '../../hooks/useSession';
import { usePermission } from '../../hooks/usePermission';
import { runInTransaction, appendOutbox } from '@invenpro/core';
import { getDb } from '../../db/schema';
import { appendLog } from '../../db/queries/log';
import { getItemById, updateItemFields, adjustStock, upsertStock } from '../../repos/items';
import { getUnitById, upsertUnit, setUnitStatus } from '../../repos/equipmentUnits';
...
export function QuickAddEditSheet({ visible, entityRef, canEdit, canDeleteItems, canAdjustStock, onClose, onSaved, onDeleted }: Props) {
  ...
  function saveItem(ref: Extract<JustAddedRef, { kind: 'item' }>) {
    ...
    try {
      runInTransaction(() => {
        // updateItemFields already mirrors this UPDATE to the outbox internally
        // (repos/items.ts routes it through itemsRepo.update()) — do not append again.
        updateItemFields(ref.id, {
          name: trimmed,
  ...
  function saveUnit(ref: Extract<JustAddedRef, { kind: 'equipment_unit' }>) {
    ...
    try {
      runInTransaction(() => {
        // upsertUnit already mirrors this write to the outbox internally
        // (repos/equipmentUnits.ts routes it through equipmentUnitsRepo.insert()) —
        // do not append again.
  ...
  function deleteUnit(ref: Extract<JustAddedRef, { kind: 'equipment_unit' }>) {
    ...
    confirmDestructive({
      ...
      onConfirm: () => {
        try {
          runInTransaction(() => {
            // setUnitStatus already mirrors this UPDATE to the outbox internally — do not append again.
            setUnitStatus(ref.id, { status: 'retired' });
            appendLog({
  ...
  function saveStock(ref: Extract<JustAddedRef, { kind: 'stock' }>) {
    ...
    try {
      runInTransaction(() => {
        if (ref.mode === 'set') {
          // upsertStock already mirrors this INSERT to the outbox internally — do not append again.
          ...
          if (delta !== 0) {
            // adjustStock already mirrors this ADJUST to the outbox internally — do not append again.
