...
import { useThemedStyles, AppInput, FieldLabel, FormScreen } from '@invenpro/ui';
...
export default function EquipmentQuickAdd({ onSaved }: Props) {
  ...
  function handleSave() {
    ...
    // Atomic write: every unit's upsert + log all-or-nothing, so a mid-batch
    // failure can't leave some units saved and others missing. upsertUnit
    // already mirrors its write to the outbox internally (repos/equipmentUnits.ts
    // routes it through equipmentUnitsRepo.insert()) — do not append again.
    try {
      runInTransaction(() => {
