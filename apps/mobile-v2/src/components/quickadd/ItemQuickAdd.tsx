...
import { getItemTypes, parseItemTypeMeta, getItemTypeColorMap } from '../../repos/taxonomy';
...
export default function ItemQuickAdd({ onSaved }: Props) {
  ...
  function handleSave() {
    ...
    // Atomic write: upsert + log all-or-nothing so a mid-flow failure can't
    // leave an orphaned item or a lost log entry. upsertItem already mirrors
    // its write to the outbox internally (repos/items.ts routes it through
    // itemsRepo.insert()) — do not append again.
    try {
      runInTransaction(() => {
        upsertItem(item);
        appendLog({
