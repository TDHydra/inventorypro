...
import { upsertLocation, getTopLevelLocations } from '../../repos/locations';
import type { Location } from '../../repos/locations';
...
import { useThemedStyles, getTheme, FieldLabel, FormScreen } from '@invenpro/ui';
...
export default function LocationQuickAdd({ onSaved }: Props) {
  ...
  function handleSave() {
    ...
    // upsertLocation already mirrors this INSERT to the outbox internally
    // (repos/locations.ts routes it through locationsRepo.mirror()) — do not append again.
    upsertLocation(loc);
    appendLog({
    ...
    // No explicit reload: appendOutbox's table bump drives the
    // useDbQuery(['locations']) read above, refreshing the parent picker.
