...
import { useThemedStyles, EmptyState } from '@invenpro/ui';
...
// DayBoardScreen.tsx (route file: app/(app)/schedule/index.tsx). Import
// mapping: '../ui/*' -> '@invenpro/ui', '../../hooks/useDataVersion'
// (useTableVersion) + '../../db/tx' -> '@invenpro/core', '../../db/queries/
// schedule' -> '../../repos/schedule'.
//
// Visible read-only to EVERYONE; write affordances (empty-cell tap, "Clear
// this slot") only render for manage_schedule editors — `canEdit` threads
// down, this screen never hard-gates the route itself (the hub tile is what's
// permission-hidden — ungated here per the station brief: "gated
// appropriately" means the WRITE affordances, matching the old app's own
// design, which never hid the route behind a permission either).
//
// Behavior change (no-self-log convention): the QuickCreateSheet handoff's
// assignJobSlot call now builds its own 'schedule_assigned' log entry
// (previously the repo layer did this internally) — wrapped in the same
// runInTransaction the repo call already opens (reentrant -> one commit).
          ...
          if (pendingCreate) {
            ...
            runInTransaction(() => {
              ...
              appendLog({
                ...
                entity_id: pendingCreate.employeeId, from_location_id: null, to_location_id: null,
                quantity: null, unit: null, job_id: created.id, note: null,
                metadata: JSON.stringify({
                  assignment_id: id, day, start_minute: pendingCreate.startMinute,
                  end_minute: pendingCreate.endMinute, assignment_kind: 'job', job_id: created.id, manager_id: null,
                }),
                device_id: null,
              });
            });
