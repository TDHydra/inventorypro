// Ported from apps/mobile/src/access/personalLocker.ts — import mapping only
// ('../db/queries/locations' → '../repos/locations', './unitGrants' →
// '../repos/access'). No self-log divergence here: the old app's
// personalLocker.ts never called appendLog either (findOrCreateLockerForUser/
// retireLocker are their own location_created/location_archived-style writes
// at the locations.ts layer, and enable/disable don't log a locker-specific
// event) — so this port matches verbatim.
//
...
import { grantUnitAccessWithDefaults } from '../repos/access';
