// Station D3: ported from apps/mobile/src/notifications/localAlerts.ts.
// Import mapping per docs/REBUILD-PORTING.md: db/appSettings + sync/outbox →
// '@invenpro/core'; db/queries/* → '../repos/*'. Wiring deviation: the old
// engine.ts called runLocalAlertChecks() in its hardcoded post-cycle block;
// v2 registers it as an afterPull hook in src/boot.ts (no `tables` filter —
// the checks are cheap, self-deduping, and the sync-stuck alert depends on
// outbox state, not on any pulled table).
...
import { getExpiringUsers } from '../repos/users';
...
export async function runLocalAlertChecks(): Promise<void> {
  try {
    ...
    // status edits on the repair detail screen (repairs/[id].tsx canEdit).
