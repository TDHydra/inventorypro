// Web shim for ./localAlerts (native uses expo-notifications). Backed by the
// browser Web Notifications API. Exports the SAME symbols with identical
// signatures so shared call sites work unchanged on web. Never throws at
// import time and degrades gracefully when Notification is unavailable or
// permission is denied (all checks become no-ops).
// Station D3: ported from apps/mobile/src/notifications/localAlerts.web.ts
// with the same import mapping as the native twin.
import {
  getAppSetting,
  setAppSetting,
  deleteAppSetting,
