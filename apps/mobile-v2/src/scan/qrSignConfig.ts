// Ported from apps/mobile/src/scan/qrSignConfig.ts. Import mapping applied:
//   '../db/appConfig'  → '@invenpro/core' (getAppConfig/setAppConfigLocal)
//   '../sync/outbox'   → '@invenpro/core' (appendOutbox)
// pushConfig below writes app_config directly + appendOutbox, same as the old
// file and the same precedent as src/db/{formMode,hiddenFields,mainStorage}.ts
// (app_config has no repo — this is a legit direct appendOutbox caller, not a
// screen).
