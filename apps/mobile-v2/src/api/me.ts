// Authed self-service /me endpoints (Station D3, ported from
// apps/mobile/src/api/me.ts). All calls are online-only: the server is
// authoritative and updates users.updated_at so the change propagates to
// every device via the normal sync pull. Callers mirror the confirmed value
// into the LOCAL users row (updateUserLocal, no outbox — the server already
// has it) so this device reflects it immediately.
//
// PIN change lives with its auth siblings in @invenpro/core (changePinOnline).
