...
// Same harness as the old app's rooms.test.ts: intercept Module._load and swap
// db/schema for a REAL sql.js database (./testDb) so these tests exercise the
// actual helpers end-to-end (writes, dedup/reactivate, outbox rows) — now also
// wired to @invenpro/core's provider/config seam, since rooms.ts writes through
// createRepository() rather than a hand-rolled write+appendOutbox pair.
...
before(async () => {
  await testDb.initTestDb(['rooms', 'outbox']);
