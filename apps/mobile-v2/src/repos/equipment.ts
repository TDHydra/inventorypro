// Ported from apps/mobile/src/db/queries/equipment.ts. Read-only module — no
// writes here in the old app either, so nothing to convert to createRepository.
//
// Import mapping applied:
//   '../schema'            → '../db/schema'
//   './taxonomy'           → './taxonomy' (partial port — see repos/taxonomy.ts)
//   './items'              → './items' (partial port — see repos/items.ts)
//   './equipmentUnits'     → './equipmentUnits' (ported alongside this file)
