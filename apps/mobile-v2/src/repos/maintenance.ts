// Ported from apps/mobile/src/db/queries/maintenance.ts.
//
// Import mapping applied:
//   '../schema'          → '../db/schema'
//   '../../sync/outbox'  → dropped; write path now goes through
//                          createRepository('maintenance_events') from '@invenpro/core'
//   './log'              → '../db/queries/log' (appendLog exists unchanged in v2,
//                          but repos/ sits one level closer to src/ than
//                          db/queries/ did, so the relative path changes)
//   '../tx'              → '@invenpro/core' (runInTransaction)
//   '../../utils/uuid'   → '../utils/uuid' (exists in v2; one less '..' — repos/
//                          is one level below src/, utils/uuid was two levels
//                          up from the old db/queries/ location)
//
// Write-path conversion: the old hand-rolled local INSERT + appendOutbox pair
// (inside its own runInTransaction, alongside an appendLog call) is replaced
// by createRepository('maintenance_events').insert(...) — a whole-row create,
// per REBUILD-PORTING.md's "whole-row upsert → repo.insert(row)" rule.
// repo.insert() opens its own transaction internally; runInTransaction is
// reentrant (a nested call joins the outer one), so wrapping it together with
// appendLog in the same outer runInTransaction still commits atomically, same
// as before.
...
const maintenanceEventsRepo = createRepository('maintenance_events');
