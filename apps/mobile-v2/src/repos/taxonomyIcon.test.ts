...
// taxonomy.ts can't load under `node --test` as-is: db/schema imports the
// native op-sqlite binding (which drags in expo) and utils/uuid imports
// react-native-get-random-values. Intercept Module._load (tsx runs this
// package as CommonJS) and swap db/schema for a REAL sql.js database (./testDb,
// which builds taxonomy_types from the manifest DDL) so getTypeIcon runs
// end-to-end against actual SQLite — including its per-category default-icon
// fallback.
...
before(async () => {
  await testDb.initTestDb(['taxonomy_types', 'outbox']);
