// Ported from apps/mobile/app/(app)/(jobs)/[id].tsx (717 ln, was a
// parenthesized group; plain route here per docs/REBUILD-PORTING.md).
//
// Import mapping applied:
//   '../../../src/db/queries/jobs' → '../../../src/repos/jobs' (includes
//     getAssignmentsForJob/getAssignableCrews/assignJobToCrew/assignJobToUser/
//     unassign — old app split these into jobAssignments.ts; Station C1
//     ported them into the same repos/jobs.ts per the brief's file layout)
...
import {
  Alert, useThemedStyles, PrimaryButton, AppInput, FormScreen, FieldLabel,
...
import { getLogForJob, appendLog, type LogEntry } from '../../../src/db/queries/log';
...
import { getAllTeams } from '../../../src/repos/teams';
...
export default function JobDetailScreen() {
  ...
  const teamOptions = useMemo((): PickerOption[] =>
    getAllTeams().map(tm => ({ id: tm.id, label: tm.name })), [refreshKey]);
  ...
  function startTeamEdit() {
    const cur = job!.team_id ? (teamOptions.find(tm => tm.id === job!.team_id) ?? null) : null;
    setTeamPick(cur);
    setTeamEditing(true);
  }
  ...
  function doAssign(kind: 'crew' | 'user', assigneeId: string, label: string) {
    ...
    // audit log atomically (repos/jobs.ts's assign()); re-picking an already-
    // active assignee is an idempotent no-op.
    try {
