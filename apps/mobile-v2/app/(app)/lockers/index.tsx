// Ported from apps/mobile/app/(app)/(lockers)/index.tsx (was a parenthesized
// group; plain route here per docs/REBUILD-PORTING.md / Station C3).
//
// Import mapping applied:
//   '../../../src/db/queries/access' (getVisibleUnits) → '../../../src/repos/access'
//   '../../../src/db/queries/users' → '../../../src/repos/users'
//   ui/EmptyState → '@invenpro/ui'
//   router pathname '/(app)/(lockers)/[id]' → '/(app)/lockers/[id]' (plain route)
// Straight port otherwise.
import { useMemo, useState } from 'react';
import { View, Text, ScrollView, TouchableOpacity, StyleSheet } from 'react-native';
import { Stack, useRouter } from 'expo-router';
import { EmptyState, useThemedStyles, type Theme } from '@invenpro/ui';
import { LockerSheet } from '../../../src/components/lockers/LockerSheet';
import { getVisibleUnits } from '../../../src/repos/access';
import { getUserById } from '../../../src/repos/users';
...
export default function LockersScreen() {
  ...
  const { units, showsAll } = useMemo(
    () => (user ? getVisibleUnits(user, 'Locker') : { units: [], showsAll: false }),
    [user?.id, refreshKey],
  );
  ...
  return (
    <>
      <Stack.Screen options={{ title: 'Lockers', headerShown: true }} />
      <ScrollView style={s.screen} contentContainerStyle={s.content}>
        {units.length === 0 ? (
          <EmptyState icon="🔒" title="No lockers yet"
            subtitle="Lockers you own, share a team with, or were granted access to show up here." />
        ) : (
          <>
            {showsAll && <Text style={s.caption}>Manager view — showing every locker.</Text>}
            {units.map(loc => (
              <TouchableOpacity key={loc.id} style={s.row}
                onPress={() => router.push({ pathname: '/(app)/lockers/[id]', params: { id: loc.id } })}>
      ...
      {infoId && <LockerSheet locationId={infoId} visible={infoOpen} onClose={() => setInfoOpen(false)} />}
    </>
  );
}
