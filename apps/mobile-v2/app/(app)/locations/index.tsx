// Ported from apps/mobile/app/(app)/(locations)/index.tsx.
//
...
//   '../../../src/sync/outbox' (appendOutbox) → dropped; upsertLocation
//     (repos/locations.ts) self-mirrors to the outbox via createRepository/mirror.
//   '../../../src/db/tx' (runInTransaction) → '@invenpro/core'
//   '../../../src/hooks/useDbQuery', useDataVersion → '@invenpro/core'
//   '../../../src/lib/themedAlert' (Alert), ui/* components, useTheme,
...
import {
  Alert, useTheme, useThemedStyles,
  ModalSheet, PrimaryButton, AppInput, FieldLabel, FilterChip, MaintenanceBanner,
} from '@invenpro/ui';
...
export default function LocationsScreen() {
  ...
  const onRefresh = useCallback(async () => {
    ...
    // drive the useDbQuery(['locations','taxonomy_types']) read above (#60/#63).
  ...
  function doCreate() {
    ...
    // Atomic: local row insert (mirrored to the outbox by upsertLocation) + log
    ...
    // drives the useDbQuery(['locations','taxonomy_types']) read above (#60/#63).
