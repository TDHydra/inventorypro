// Station D2: ported from apps/mobile/app/(app)/(media)/index.tsx (the
// (media) route group flattens to media/ in the rebuild). Import mapping:
// ui kit/Alert/multiselect → '@invenpro/ui'; useDbQuery/syncNow/
// runInTransaction → '@invenpro/core'; db/queries/media → repos/media. v2
// repos don't self-log, so the bulk delete wraps
// runInTransaction(deleteBulkMedia + one media_deleted log row per id) —
// matching the old deleteBulkMedia's per-id logging exactly.
...
import {
  ...
  FilterChip, SearchInput, EmptyState, Fab, ModalSheet, ErrorView,
  useMultiSelect, BulkActionBar, type BulkAction,
} from '@invenpro/ui';
import { useDbQuery, syncNow, runInTransaction } from '@invenpro/core';
...
} from '../../../src/repos/media';
import { logMediaAction } from '../../../src/media/mediaLog';
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            runInTransaction(() => {
              deleteBulkMedia(ids);
              for (const id of ids) logMediaAction('media_deleted', id, user?.id ?? null, null);
            });
            reloadWindow();
            ms.exit();
