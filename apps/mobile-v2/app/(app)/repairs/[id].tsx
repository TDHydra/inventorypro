/**
 * Repair detail. Ported from apps/mobile/app/(app)/(repairs)/[id].tsx (974
 * ln, Station C4). Import mapping applied (docs/REBUILD-PORTING.md):
 *   '../../../src/db/queries/repairs' → '../../../src/repos/repairs'
 *   '../../../src/db/queries/taxonomy' → '../../../src/repos/taxonomy'
 *   '../../../src/db/queries/equipmentUnits' → '../../../src/repos/equipmentUnits'
 *   '../../../src/db/queries/locations' → '../../../src/repos/locations'
 *   '../../../src/db/queries/users' → '../../../src/repos/users'
 *   '../../../src/db/queries/items' → '../../../src/repos/items'
 *   '../../../src/db/queries/log' → unchanged (log.ts wasn't moved to repos/)
 *   ui/*, useThemedStyles, Alert → '@invenpro/ui'
 *   '/(app)/(repairs)/[id]' self-route / '/(app)/(inventory)/[id]' /
 *     '/(app)/(locations)/[id]' → '/(app)/repairs/[id]' / '/(app)/inventory/[id]'
 *     / '/(app)/locations/[id]' (plain routes, Station C4)
 *   PriorRepairsCard → './repairs/PriorRepairsCard' → ported alongside this
 *     screen this station (src/components/repairs/PriorRepairsCard.tsx)
 *
 * Equipment repair auto-complete (Station C4 brief item 1): completing a
 * ticket on an equipment_unit entity prompts a return-to-service location
 * (confirmReturn below) and reopening a completed ticket drives the unit
 * back to in_repair (pickStatus below) — straight port of the old app's
 * behavior. setUnitStatus (repos/equipmentUnits.ts) already self-mirrors its
 * own outbox UPDATE, so the old app's manual outboxUnit()/appendOutbox calls
 * are dropped entirely (same simplification already applied in
 ...
 *     "Status → <label>" notes; the same information (in more detail) is
 *     already visible in the ActivityFeed "History" section below, so this
 *     is cut as redundant polish rather than a scope gap — no wave marker
 *     needed, since no brief scope item names this visualization.
 */
...
import { runInTransaction, useTableVersion, useDbQuery } from '@invenpro/core';
...
export default function RepairDetailScreen() {
  ...
  // combo with edit_inventory but neither checkin/checkout would otherwise strand
  // the outbox entry as a permanent conflict. Mirrors the same gate applied to
  // the quick-add stock sheet.
  ...
  // via useDbQuery/repair_steps so a local addRepairStep or a synced-in step
  // from another device re-renders this list without a manual reload().
  ...
  // adjustStock (repos/items.ts) already mirrors its own outbox ADJUST.
  ...
  function pickStatus(label: string) {
    ...
    const isTerminal = isTerminalStatus(label);
    ...
    // setUnitStatus (repos/equipmentUnits.ts) already self-mirrors its own
    // outbox UPDATE — no manual appendOutbox call needed here (unlike the old
    // app), matching the same simplification made in equipment/[id].tsx.
    try {
      runInTransaction(() => {
        updateRepairStatus(repair.id, label, isTerminal);
        logStatus(
          isTerminal ? 'repair_completed' : 'repair_status_changed',
          `Status → ${label}`,
        );
        if (!isTerminal && wasCompleted && repair.entity_type === 'equipment_unit') {
          // Reopening a completed repair drives the unit back to in_repair
          // (symmetric with create-time auto-drive) so it isn't "available"
          // with an open ticket.
          setUnitStatus(repair.entity_id, { status: 'in_repair', notes: null });
        }
      });
    } catch (err) {
      Alert.alert(
    ...
    if (isTerminal && repair.entity_type === 'equipment_unit') {
      // Completing a ticket on an equipment unit returns it to service —
      // prompt for the return location now that the status write has committed.
      setReturnLoc(null);
      setReturnShelf(null);
            ...
            <View style={s.chipWrap}>
              ...
              {!!dueAt && (
                <TouchableOpacity style={s.expiryChip} onPress={() => setDueAt(null)}>
                  <Text style={[s.expiryChipText, s.expiryClearText]}>Clear</Text>
                </TouchableOpacity>
              )}
            </View>
          </View>
...
const makeStyles = (t: Theme) => StyleSheet.create({
  ...
  expiryClearText: { color: t.colors.danger },
