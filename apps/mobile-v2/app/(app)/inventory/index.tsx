...
import {
  useTheme, useThemedStyles, FilterChip, SearchHeader, confirmSheet,
  ModalSheet, AppInput, FieldLabel, PrimaryButton, Fab, BulkActionBar,
  type BulkAction, useMultiSelect,
} from '@invenpro/ui';
import { useTableVersion, syncNow } from '@invenpro/core';
import { Stack, useRouter } from 'expo-router';
import { ItemCard } from '../../../src/components/ItemCard';
import { QuickAddBanner } from '../../../src/components/QuickAddBanner';
import { searchItems, updateItemFields, getDistinctValues, deleteItems } from '../../../src/repos/items';
import { getItemTypes, getItemTypeColorMap } from '../../../src/repos/taxonomy';
...
export default function InventoryScreen() {
  ...
  const tableVersion = useTableVersion(INVENTORY_TABLES);
  ...
  const filterChips = useMemo(
    () => [
      ...
      ...getItemTypes().map(ty => ({ id: ty.id, label: ty.icon ? `${ty.icon} ${ty.label}` : ty.label })),
    ],
    [refreshKey],
  );
  ...
  // (tableVersion bumps), so an already-open list refreshes without the user
  // pulling to refresh. Deliberately keyed only on tableVersion — query/filter
  ...
  useEffect(() => {
    ...
    // doesn't truncate an infinite-scrolled list back to the first page.
  ...
  const itemTypeOptions = useMemo<PickerOption[]>(
    () => getItemTypes().map(ty => ({ id: ty.label, label: ty.icon ? `${ty.icon} ${ty.label}` : ty.label })),
    [refreshKey],
  );
  ...
  // Each handler mirrors inventory/[id].tsx: updateItemFields already self-mirrors
  // to the outbox (repos/items.ts) — no manual appendOutbox here.
  ...
  const applyMinQty = useCallback(() => {
    ...
    for (const id of Array.from(ms.selected)) {
      updateItemFields(id, { min_qty_alert: n });
            ...
            loading ? null : (
              <View style={s.empty}>
                ...
                <PermissionGate permission="edit_inventory">
                  <TouchableOpacity
                    style={s.addItemBtn}
                    onPress={() => router.push({ pathname: '/(app)/quickadd/[sheet]', params: { sheet: 'item' } })}
                  >
