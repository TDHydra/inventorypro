// Ported from apps/mobile/app/(app)/(inventory)/scan.tsx as a hub-level
// surface (plan: "hub scanner absorbing old scan.tsx"). Import mapping + route
// mapping applied (no parenthesized groups except (app)/(auth)):
//   (app)/(equipment)/[id] → (app)/equipment/[id]  (unbuilt this wave — sibling)
//   (app)/(inventory)/[id] → (app)/inventory/[id]  (unbuilt this wave — sibling)
//   (app)/(locations)/[id] → (app)/locations/[id]  (unbuilt this wave — sibling)
//   (app)/(inventory)/add  → (app)/quickadd/[sheet] item (add.tsx not ported)
//   (app)/(checkout)       → (app)/checkout        (this wave — same agent)
...
import { getItemByBarcode, getItemById } from '../../src/repos/items';
import { getUnitByTag } from '../../src/repos/equipmentUnits';
        ...
        Alert.alert(
          ...
          [
            { text: 'Try Again' },
            {
              text: 'Add to Catalog',
              onPress: () =>
                // add.tsx wasn't ported — the item quick-add sheet (which reads
                // the barcode param) is its replacement.
                router.push({ pathname: '/(app)/quickadd/[sheet]', params: { sheet: 'item', barcode: resolved.code } }),
            },
          ]
        );
      }
