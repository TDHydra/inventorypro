...
// canvas/DOM dependency. This ambient declaration is program-global, so the web
// shim (printLabel.web.ts) reuses it without redeclaring.
...
export interface PrintLabelOpts {
   ...
   * is now generated offline on-device, so this URL is only parsed to recover
...
async function mediaSvg(payload: string, spec: TemplateSpec, format: BarcodeFormat): Promise<string> {
  if (format === 'code128') {
    // Keep modules crisp: at least ~1.5px per module, and a strip ~2.2× tall/wide
    // relative to the QR footprint so it reads on the same stock.
 ...
 * QR (or Code 128) symbol embedded inline, then invoke the OS print dialog.
 */
 ...
 * hand it to the OS print dialog in a single job. `format` is the default
