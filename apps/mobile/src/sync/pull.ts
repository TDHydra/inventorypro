...
const TABLE_UPSERT_SQL: Record<string, string> = {
  ...
  users: `INSERT OR REPLACE INTO users (id, name, role, pin_length_required, pin_set, permission_overrides, active, expires_at, created_at, updated_at, email, dashboard_preset_id, is_test, enrollment_code_public) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
...
function rowToValues(table: string, row: Record<string, unknown>): unknown[] {
  switch (table) {
    ...
    case 'users': return [row.id, row.name, row.role, row.pin_length_required, row.pin_set ? 1 : 0, JSON.stringify(row.permission_overrides ?? {}), row.active ? 1 : 0, row.expires_at ?? null, row.created_at, row.updated_at, row.email ?? null, row.dashboard_preset_id ?? null, row.is_test ? 1 : 0, row.enrollment_code_public ?? null];
