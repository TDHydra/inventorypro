...
const TABLE_UPSERT_SQL: Record<string, string> = {
  ...
  media: `INSERT OR REPLACE INTO media (id, entity_type, entity_id, media_type, url, thumbnail_url, caption, is_primary, uploaded_by, created_at, location_note, updated_at, audience, audience_user_ids, room_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
  app_config: `INSERT OR REPLACE INTO app_config (key, value, updated_at) VALUES (?, ?, ?)`,
  // dashboard_prefs (#193/#196, migration 060): personal layout + starred
  // widgets JSON blob, alongside theme.
  user_prefs: `INSERT OR REPLACE INTO user_prefs (user_id, theme, dashboard_prefs, updated_at) VALUES (?, ?, ?, ?)`,
  ...
  repair_steps: `INSERT OR REPLACE INTO repair_steps (id, repair_id, action, result, created_by, created_at, updated_at) VALUES (?,?,?,?,?,?,?)`,
...
function rowToValues(table: string, row: Record<string, unknown>): unknown[] {
  switch (table) {
    ...
    case 'user_prefs': return [row.user_id, row.theme ?? null, row.dashboard_prefs ?? null, row.updated_at];
    ...
    case 'repair_steps': return [row.id, row.repair_id, row.action, row.result ?? null, row.created_by ?? null, row.created_at, row.updated_at];
