...
type PushResult = { ok: string[]; conflicts: Array<{ id: string; error: string; code: string }> };
...
// ── locker_access: dropped from the v2 manifest ─────────────────────────────
// The rebuild removed locker_access from the push allowlist (PUSH_TABLES —
// unit_access is the replacement model), so EVERY push against it now rejects
// at the allowlist step with permanent wording, regardless of ownership or
// role. The old #126 owner/org-authority guard tests lived here; the ADJUST
// negative-delta locker guard (which still reads the table until migration
// >=082 drops it) keeps its own tests above.

test('locker_access: not allowlisted — every push rejects permanently, even for owner or org authority', async () => {
  for (const opts of [
    { callerRole: 'construction_crew', lockerOwner: CALLER },
    { callerRole: 'full_admin', lockerOwner: OTHER },
  ] as const) {
    const pg = fakePg({ ...opts });
    const body = await push(pg, [
      { operation: 'INSERT', table_name: 'locker_access', payload: { location_id: 'loc-1', user_id: OTHER, created_at: NOW } },
    ]);
    assert.deepEqual(body.ok, []);
    assert.equal(body.conflicts[0].error, 'Table not allowed');
    assert.equal(body.conflicts[0].code, 'NOT_ALLOWED');
    assert.match(body.conflicts[0].error, PERMANENT);
    assert.ok(!pg.queries.some(q => q.sql.includes('INSERT INTO locker_access')),
      'the write must never reach SQL');
  }
});
