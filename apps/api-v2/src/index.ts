...
async function build() {
  ...
  // failOnSchemaDrift: the manifest (packages/core) and the live schema must
  // agree at boot — serving a manifest table PG doesn't have would 500 mid-sync.
  // Tests register the plugin directly with partial pg stubs and leave it off.
  await fastify.register(syncRoutes, { prefix: '/sync', failOnSchemaDrift: true });
