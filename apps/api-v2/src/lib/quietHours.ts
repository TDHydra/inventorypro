...
// apps/mobile/src/notifications/quietHours.ts verbatim (kept in sync
// intentionally, same as the redact.ts/telemetry.ts allowlist-mirroring
// convention) — quiet hours are enforced independently on both sides: the
// server gates push delivery (notifications.ts's deliver() + the direct chat
// sendPush in routes/sync.ts), the client gates its own local alerts
// (runLocalAlertChecks).
//
// Storage: user_prefs.quiet_hours_start/_end are UTC-minutes-since-midnight
// (0-1439), computed by the MOBILE CLIENT at save time from local wall-clock
// + the device's current UTC offset. There is no timezone column anywhere in
// this schema (verified by grep), so this is a deliberate correctness
// tradeoff: a user who travels or crosses a DST boundary keeps the OLD offset
// baked into their saved window until they resave the setting on their
// device. NULL/NULL = disabled (the same "never set" convention
// theme/dashboard_layout already use).
