...
export async function setPinFirstTime(userId: string, pin: string, enrollmentCode: string): Promise<AuthResult> {
  const res = await fetch(`${apiBase()}/auth/set-pin`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
 ...
 *
 * `getValidJwt` comes through the injected auth seam (../config) — the session
 * module itself is app-side (it wraps the platform secure store).
 */
export async function changePinOnline(currentPin: string, newPin: string): Promise<void> {
  ...
  const res = await fetch(`${apiBase()}/me/change-pin`, {
    method: 'POST',
