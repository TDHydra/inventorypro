...
## Features — pending (2026-07-06)
- [ ] **Shelf picker + proximity location default on the equipment/initial-units add flow — and extract both into reusable components.** Two parts:
  - **Feature:** the equipment/units add screen (`app/(app)/(equipment)/add.tsx` + `src/components/quickadd/EquipmentQuickAdd.tsx`) should place a unit's **initial location** the same way the other screens do — detect the user's current position (`src/hooks/useCurrentPosition.ts`) and pre-select / sort locations by proximity (`src/location/proximity.ts`, as `DestinationPicker`/`LocationSuggestionBanner`/checkout/`add.tsx` already do) so you start "as close as it can" to the right spot — and reveal a **shelf field only when the chosen location `has_shelves`**, with the dynamic-filtered shelf typeahead + create-if-missing (`getShelvesForParent`/`findOrCreateShelf` + the `{id:'__new__'}` sentinel from `queries/locations.ts`), identical to `add.tsx`/`StockQuickAdd`.
  - **Refactor (the real win):** this location-picker-with-proximity-default + `has_shelves`-gated shelf-picker pattern is now hand-copied into `add.tsx`, `StockQuickAdd.tsx`, `EquipmentQuickAdd.tsx` (and wanted here). Extract it into **one reusable component** — e.g. `<LocationShelfPicker value onChange … />` (encapsulating: the location `SearchablePicker` with proximity ordering, the gated shelf `SearchablePicker`, and the submit-time resolution to a concrete location/shelf id via `findOrCreateShelf`) plus maybe a `useProximityLocations()` hook — and swap every screen over to it. First extraction of the broader effort below.
- [ ] **⭐ Componentize the app — extract the repeated cross-screen patterns into a shared component library.** The copy-paste runs through **Jobs, Users, Items, Teams, Locations, Equipment, Repairs — basically every screen**. Do a DRY pass: catalog the duplicated patterns and turn each into ONE reusable, well-typed component, then swap every screen onto it (delete the hand-rolled copies). Known repeat offenders to consolidate:
  - **Taxonomy/type pickers** — team-type / job-type / location-type / item-category / repair-status are all `SearchablePicker` over `getTaxonomyTypes(category)` + add-new-type; → a single `<TaxonomyPicker category … />`.
  - **Quick-Add sheets** — Item/Job/Team/User/Equipment/Location/Stock/Repair sheets share the `QuickAddScreenShell` + field/validation/save/outbox shape; factor the common scaffolding.
  - **Entity edit modals** — the `ModalSheet` + labelled-field + Save/Cancel + `updateXFields`→outbox pattern repeated per entity; → a shared edit-form primitive.
  - **List rows / cards** (`ItemCard` and the per-entity list rows), **search bar + filter chips** (inventory/logs/jobs), and the **multi-select `BulkActionBar` flow** (inventory/equipment/users/jobs) — extract shared versions.
  - **Location + shelf picker** (the item above), and the already-extracted **`DragList`** (reorder) is the model to follow.
  Approach: incremental — rank by duplication count, extract the highest-value ones first (TaxonomyPicker + LocationShelfPicker + the edit-form scaffold), verify each swap with tsc + hotload before the next. Big cleanup; reduces bug surface + future headache exactly as the user noted.


- [ ] **🔒 Nobody can change roles/permissions ABOVE their own level — anywhere, no matter what.** Generalize the existing partial guards into one consistent **tier-based escalation rule**, enforced on **every** write path (REST + **sync outbox** + client UI): a user may never (a) change the **role** of a target whose tier ≥ their own, (b) grant/enable a **role** at or above their own tier, (c) grant a **permission they don't personally hold**, or (d) edit the **permission matrix / role_settings** for a role at or above their tier. Today only pieces exist: `PRIVILEGED_ROLES` + `requiresRolesPermForTarget` + `USERS_ROLE_GATED` (`apps/api/src/lib/syncPolicy.ts`), the `PRIVILEGED_ROLES` PATCH check in `apps/api/src/routes/users.ts`, and `sanitizeTeamOverrides` ("only grant perms you hold"). Replace the hardcoded-`PRIVILEGED_ROLES` special case with a **general `ROLE_TIER`-comparison** (`src/constants/roles.ts`) so it covers *all* tiers, not just full_admin/franchise_manager — e.g. an office_manager can't touch a head_of_construction, a head can't touch a franchise_manager, etc. Cover: `/users` PATCH (role/active/expires/overrides), `role_settings` writes (Roles & Permissions screen), `team_members` overrides, and the equivalent **sync** writes to `users`/`role_settings`/`team_members` (server resolves the caller's tier from the DB, never trusts the JWT). Client-side: gray out / hide the controls in the Users + Roles admin screens for any target at ≥ the actor's tier. Add a shared server helper `canActOnTarget(callerRole, targetRole)` + unit tests covering every tier pair. **Security-critical — belongs on the API-hardening track.**
- [ ] **Condense "Notification Routing" into a single Settings button/screen.** The Notification Routing config is currently an inline section in `app/(app)/(admin)/settings.tsx` (under the `{isAdmin}` System block, alongside Notification Triggers). Condense it to **one clearly-labelled row/button** in Settings that opens its own dedicated screen (`app/(app)/(admin)/notification-routing.tsx`), presented so **anyone in a leadership role can understand what goes where** — i.e. a plain "when X happens → these people/roles get notified" mapping view rather than the current dense inline controls. Keep the existing routing data model + `resolveRecipients` logic (`apps/api/src/lib/notifications.ts`); this is a UI/IA refactor (extract + simplify + link from a button), not a data change. Gate the same as today (`system_settings`/tier-4).
- [ ] **⭐ Chat / direct + group messaging with push (new feature).** In-app **direct messages** (1:1) and **group messages** (named or ad-hoc participant sets), with **push notifications** on new messages. **Per-message urgency:** a sender marks a message **Urgent** (default — always pushes) or **Regular** (softer: quieter/badge-only or user-muteable — "not a big deal"); recipients can adjust their own per-conversation notification preference (all / urgent-only / muted). Design notes for later scoping: ride the existing **offline-first sync + push** rails rather than adding realtime WebSockets — model `conversations` + `conversation_participants` + `messages` as synced tables (new migrations both sides + pull.ts/sync.ts/syncPolicy/fullDownload plumbing), so messages propagate on the normal sync pull (near-real-time while active) and **reuse the shipped notifications/FCM push platform** (`apps/api/src/lib/push.ts` + the notifications table/routing) to deliver the urgent/regular push. New mobile screens (conversation list + thread + new-message/compose + participant picker reusing `SearchablePicker`), a per-user unread/badge count, and a per-conversation notification-pref setting. Consider: read receipts, typing indicators (optional/later), message retention, and that `messages` are user-authored (attribution forced server-side like other synced writes). Big feature — will need its own spec + phased build (data model → sync → UI → push wiring → prefs).
- [ ] **Enrich the Activity Logs + the Settings→System Analytics with more detail.** Two related deepenings:
  - **Activity Logs** (`app/(app)/(logs)`, backed by the immutable `activity_log` table — `action`, `entity_type`, `entity_id`, `user_id`, `team_id`, `job_id`, `from_location_id`/`to_location_id`, `quantity`, `unit`, `note`, `metadata`, `latitude/longitude`, `created_at`): surface more of what each entry actually captured — resolve the id columns to human names (user/team/job/location labels), show **before→after / what-changed** detail where the `metadata` JSON carries it, richer per-entry expansion, and stronger **filtering/search** (by user, action type, entity, team, date range) + optional map/coords view (coords are already stamped). Reuse `resolveLabels`/entity queries for the id→name resolution; don't alter the append-only model.
  - **Analytics** (`app/(app)/(admin)/analytics.tsx`, `GET /telemetry/summary`, `system_settings`-gated; today: 24h/7d/30d stat tiles + bar lists over `telemetry_events` screen/error/action/audit): add more **metrics + breakdowns** — e.g. per-user / per-role / per-team activity, top actions & screens, error trends, checkout/checkin/stock-movement volumes, active-device counts, and drill-downs — extending `getTelemetrySummary()` (`apps/api/src/…`) with the new aggregations and the screen with the matching tiles/charts (use the `dataviz` skill for any new charts). Scope the exact metric set with the user before building.

## Fixes — pending (2026-07-05)
- [x] **Team/role-based configurable dashboards (⭐ new feature).** *(DONE + DEPLOYED 2026-07-06, commits `e88f4d0`+`7003029`: synced `dashboard_presets` table + `users`/`role_settings.dashboard_preset_id` (migrations mob 033/API 039); `src/dashboard/{widgets.ts WIDGET_REGISTRY+DEFAULT_LAYOUT, store.ts reactive resolve}`; hub renders resolved layout, default = today's dashboard, every tile still `PermissionGate`-wrapped; admin editor `(admin)/dashboards.tsx` w/ drag-reorder `DragList` + add-widget + per-block config + role-default map; per-user picker in Users. Per-user assignment defaulting from role; custom ordered full/half-width block layouts.)*
- [x] **Reset + email the one-time enrollment ("access") code for users who have never logged in.** *(DONE + DEPLOYED 2026-07-06 — nodemailer `apps/api/src/lib/mail.ts`, `users.email` (mig 038), `POST /users/:id/reset-enrollment-code` 6-digit + `/auth/set-pin` `^[0-9]{6}$`, mobile Users email field + reset-&-email button. ⚠️ needs `SMTP_*` env on the Unraid `.env` to actually send.)* Today an admin-created user gets a one-time enrollment code (`users.enrollment_code_hash`, bcrypt, server-only, never synced — migration 026) that they enter along with a new PIN via `POST /auth/set-pin` (auth.ts:205) to complete first-login; `pin_set=false` routes them to the set-PIN screen. Gaps to close: **(1) Reset/regenerate** — an admin action (on `app/(app)/(admin)/users.tsx`, gated by `manage_users`/`set_pins`) to re-issue the enrollment code for any user who hasn't set a PIN yet (`pin_set=false`), invalidating the old code (rehash + store). Add a `POST /users/:id/reset-enrollment-code` (or extend the existing admin PIN-reset path) that generates a fresh code. **(2) 6-digit code** — generate the enrollment code as a **6-digit numeric** code (tighten the current `minLength 4/maxLength 12` on `/auth/set-pin`'s `enrollment_code` to a 6-digit format, and make the generator emit 6 digits). **(3) Email delivery** — send the new code to the user's email. NOTE: the API has **no email/SMTP capability today** — this needs a mail sender added (SMTP creds or a provider like Resend/SES/Postmark, config via env on the Unraid stack; users need an `email` column/value — verify `users.email` exists). **(4) User sets their own PIN** — the existing `/auth/set-pin` first-login flow already lets the user choose their own PIN once they enter the code; reuse it (the user types the emailed 6-digit code + picks a PIN). Touch points: `apps/api/src/routes/users.ts` (admin reset endpoint + code generation), `apps/api/src/routes/auth.ts` (`/auth/set-pin` code-format validation), a new mail module in `apps/api/src/lib/`, and `app/(app)/(admin)/users.tsx` (admin "Reset access code / email it" button, shown for not-yet-enrolled users). Decide: show the code to the admin too (for manual/offline delivery) vs email-only.
- [x] **Sub-areas should use sub-area location types, not the top-level type list.** *(DONE + DEPLOYED 2026-07-06 — new `location_subtype` taxonomy category (Closet/Section/Storage/Shelf/Area/Bin/Rack) seeded via migrations mob 030/API 036 (fixed UUIDs both sides); location edit + create pickers switch to sub-area types when `parent_id != null`; managed in a new "Location Sub-Types" Manage Types section.)* When a location being created/edited has a **parent** (i.e. it's a sub-area / child location, `parent_id != null`), the location-type picker should NOT show the same `location_type` taxonomy the top-level locations use (Shop, Vehicle, Warehouse, Job Site, …) — a sub-area is a *part of* its parent, so those types don't make sense. Offer a **sub-area-appropriate set instead** (e.g. **Closet, Section, Storage, Shelf, Area, Bin, Rack**). Implementation options: (a) a separate taxonomy category `location_subtype` seeded with the sub-area types, surfaced by the type picker only when `parent_id` is set; or (b) tag each `location_type` row (via `meta`) as top-level / sub-area / both, and filter the picker by context. Prefer a distinct category so admins can manage the two lists independently in Manage Types. Touch points: `queries/taxonomy.ts` `getLocationTypes`/`LOCATION_TYPE`, the location edit picker (`app/(app)/(locations)/[id].tsx`), and the location quick-add. Ties in with the shelf item below (shelves are already a sub-area concept).
