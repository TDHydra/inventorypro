...
- [~] Taxonomy hardening: migrate entity `type` from **label → FK id** (rename-propagation safety) — **#74**
  ...
  - [ ] **Phase 3b/3c** (optional polish, not a correctness gap): (b) extend the Phase-1 FK pattern (`*_id` column + dual-write + resolve-on-push + `resolveLabels` read) to `repairs.status` + `maintenance_events.type` so renaming a repair status / maintenance type shows immediately; optionally stop writing the label caches (keep dual-write — dropping it is risky). (c) `getDistinctValues('category')` autocomplete still returns raw cache labels (low priority). Both are lower-traffic than teams/jobs/locations/items and add sync-core surface — do only if the consistency is wanted.
