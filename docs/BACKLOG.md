...
- [ ] **Team/role-based configurable dashboards (⭐ new feature).** Different teams/roles should get a different **home/hub dashboard** streamlined for their work — e.g. a **Mitigation** team sees a different dashboard than **Contents**, **Construction**, or **HR** (each showing the tiles/actions relevant to that role, hiding the rest). Admins configure, in **Settings**, which dashboard each user gets (per-user assignment, likely defaulting from the user's team/role). Design (to confirm): named **dashboard presets** (Mitigation/Contents/Construction/HR/…), each = an ordered/enabled set of the existing hub tiles/sections; a preset is assignable per user (admin-managed) with a sensible default by team-type or role; stored in synced config (app_config or a new `dashboards` + assignment table) so it flows to every device; edited from an admin Settings screen. Reuses the existing hub tile/`PermissionGate` model (permissions still gate actions; the dashboard just curates *layout/visibility* on top). Open design Qs being scoped: assignment granularity (per-user vs per-team vs per-role), whether presets are curated tile-sets vs fully custom layouts, and who may edit.
